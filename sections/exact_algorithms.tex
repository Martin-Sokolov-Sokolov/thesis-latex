\section{Establishing Fixed-Parameter Tractability for FVS-4CL and FVS-BCL}
\label{section:exact-alg}

In this section, we present an algorithm that finds exact solutions to the FVS-4CL problem in each subgraph. 
For each subgraph, we assume that the treewidth is bounded, allowing our algorithm to achieve FPT running time.
...
\subsection{FPT Result for FVS-4CL via MSOL}
...
\subsection{FPT Result for Weighted FVS-BCL via MSOL}
...
\subsection{FPT Result for FVS-4CL via Dynamic Programming on Nice Tree Decompositions}
\label{alg:dp}

The following algorithm was changed midway writing the thesis. Initially it used multiple flag 
sets $\mathcal{F}_1, \mathcal{F}_2, \dots \mathcal{F}_{\rho-3}$. We will explain the idea in the 
end of this Subsection. The reason is that the author found that this problem was already stated 
in Cygan et al.~\cite{cygan2015parameterized}[Problem 7.21]. We extend their idea 
and give formal definitions of the problem.
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        vertices $v,u \in X_i: 
        (v, u) \in \mathcal{F}$ if they have a common neighbour $w \in V_i \backslash X_i$, such that 
        $w \notin F_i$.
    \end{enumerate}
...
\begin{align*}
c[i, I, \mathcal{F}] 
&= \min_{\substack{\mathcal{F}_1 \cup \mathcal{F}_2 = \mathcal{F}}} 
\left( 
  c[j_1, I, \mathcal{F}_1] + c[j_2, I, \mathcal{F}_2] - w(I) 
\right), \\
\text{where} \quad
c[t, I, \mathcal{F}] 
&= \infty \quad \text{if the state } (t, I, \mathcal{F}) \text{ is infeasible}.
\end{align*}

A state \( (t, I, \mathcal{F}) \) at a \emph{join node} \( t \), 
with left child \( t_1 \) and 
right child \( t_2 \), is \emph{infeasible} if there exist vertices \( u, w \in X_t \) such that:
\begin{itemize}
    \item \( u, w \notin I \),
    \item \( \{u, w\} \in \mathcal{F}_1 \)
    \item \( \{u, w\} \in \mathcal{F}_2 \).
\end{itemize}

In this case, since neither \( u \) nor \( w \) are in \( I \), but both appear in the flag sets 
of both children, they both have forgotten adjacent vertices that have not had their weights included 
in the table. Therefore, a four cycle emerges where none of the vertices are included in the partial 
solution and thus we assign the special $\infty$ value to that table value.

\[
c[t, I, \mathcal{F}] = \infty.
\]

We refer to examples to better understand the table population in the Appendix~\ref{app:first} and \ref{app:second}.

It is important to note that this method of solving FVS-4CL on nice tree decompositions can 
be extended to the more general FVS-BCL problem. One would need to hold additional flag sets 
$\mathcal{F}_1, \mathcal{F}_2, \dots \mathcal{F}_{\rho-3}$ for the forgotten vertices that are 
not in the partial solution and are of distance $d_1, d_2, \dots d_{\rho-3}$ to the vertices in 
the bag. This way of solving the FVS-BCL problem, would however, incur additional running time 
and space. In the final section of this thesis we propose other methods worth exploring in order 
to devise better algorithms.