...
In this section we initiate the study of certified algorithms for FVS-BCL and r-Dominating Set.
...
in \cite{bumpus_et_al:LIPIcs.SWAT.2024.19, venne2022designing}. 
 ...
 Unless specified otherwise, let $\Pi$ be a vertex-minimization problem.

\subsection{Algorithms for m-stitching and FVS-m-stitching}

As we mentioned before, in this section we refer to Bumpus et al. \cite{bumpus_et_al:LIPIcs.SWAT.2024.19} 
...
the cycles we are trying to break. 
 ...
 To introduce $\Pi$-m-stitching, one needs to first understand m-stitching and m-stitchable problems. In the stitching 
...
\subsection{r-Dominating set-m-stitching}
...
\begin{mydefinition}[r-DS-(r+1)-stitching]{Lemma}
    ...
    $(G, w : V(G) \to \mathbb{N})$ in time $f(tw(G)) \cdot |V(G)|^{O(1)}$. Then 
    Algorithm~\ref{alg:stitch-r-DS} solves $r$-DS in time 
    $f(tw(N^{r+1}_{G}[J])) \cdot V(G)^{O(1)}$.
