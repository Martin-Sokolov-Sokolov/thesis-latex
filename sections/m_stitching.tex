\section{m-stitching and FVS-BCL-m-stitching}
...
\subsection{Algorithms for m-stitching and FVS-m-stitching}
...
\noindent To introduce $\Pi$-m-stitching, one needs to first understand m-stitching and m-stitchable problems. In the stitching 
...
\noindent The following theorem is the main result in 
Bumpus et al.~\cite[Theorem 1.7]{bumpus_et_al:LIPIcs.SWAT.2024.19}.

\refstepcounter{definition}
\begin{mydefinition}[]{Theorem.}
    \label{theorem-main}
...
\noindent The algorithm $A$ on line~\ref{alg:stitch-fvs-4cl} refers to calling 
    ...
    \begin{figure}[H]
        ...
        \begin{minipage}{0.50\textwidth}
            ...
            \begin{tikzpicture}[scale=0.8]
                ...
                \draw[blue, thick] (new31) circle(0.5);
...
The running time of the algorithm is dominated by the call of algorithm $A$. Therefore, 
for an integer $m$ and some computable function $f$, the running time is 
$O(tw(N^{m}_{G}[J])) \cdot |V(G)|^{O(1)}$.