...
We refer to the literature in \cite{bumpus_et_al:LIPIcs.SWAT.2024.19}.
...
\begin{mydefinition}[$m$-stitching]{Definition}
    \label{m-stitch}
...
\refstepcounter{definition}
\begin{mydefinition}[m-stitchable]{Definition}
    \label{m-stitchable}
...
\refstepcounter{definition}
\begin{mydefinition}[Guessable]{Definition}
    \label{guessable}

     A problem $\Pi$ is \emph{guessable} if there is an algorithm that outputs a feasible solution 
...
\refstepcounter{definition}
\begin{mydefinition}[Meta-Theorem for Minor-Closed Graph Classes]{Theorem.}
    \label{theorem-main}

    Let $\mathcal{G}$ be a minor-closed graph class whose local treewidth is bounded above by a linear 
    ...
    on any input $(G, w : V(G) \to \mathbb{N})$ with $G \in \mathcal{G}$ and 
    polynomially-bounded weights.

\end{mydefinition}

The proof of theorem \ref{theorem-main} is beyond the scope of this thesis. In this section 
...
The algorithm $A$ on Line~\ref{alg:stitch-fvs-4cl} refers to using 
Corollary~\ref{cor:msol-fvs-rho} as a result.
...
\begin{mydefinition}[FVS-4CL-2-stitching]{Lemma}
    ...
    in time $f(t) \cdot |V(G)|^{\mathcal{O}(1)}$ where $t$ is the treewidth of $G$. 
...
\begin{enumerate}
    ...
    \begin{figure}[H]
        ...
        \begin{minipage}{0.49\textwidth}
            ...
            \end{tikzpicture}
            \caption{$\forall v \in \{v_1,v_2,v_3,v_4\}$, such that $v_1v_2v_3v_4$ is a four 
            cycle: $v \notin J$}
...
have to take a different approach as we will do with the $r$-Dominating Set problem.
...
FVS-BCL-$\rho$-stitching in time $f(\tw, \rho) \cdot n^{\mathcal{O}(1)}$.

\subsection{$r$-Dominating Set and $(2r)$-stitching}
...
argue that in the case of that problem the parameter $m$ has to be set to $2r$ in order 
...
\begin{mydefinition}[\boldsymbol{$r$}-DS is guessable]{Lemma}
    ...
    For any planar graph $G$, the set $V(G)$ is a feasible solution to $r$-Dominating Set.
...
\refstepcounter{definition}
\begin{mydefinition}[$r$-Dominating Set is $(2r)$-stitchable]{Lemma}
    \label{lemma-r-DS-stitchable}

    Let $(G, w)$ be any instance to the $r$-DS problem, $J \subseteq V(G)$, some subset
    of the vertices in the graph 
    and $S_1$ and $S_2$ any two feasible solutions to the problem in $G$. Then the set $S_3$:
    \[S_3 := (S_1 \backslash J) \cup (S_2 \cap N^{2r}_{G} \left[J\right]).\]
    is a feasible solution to the problem.
...
\begin{proof}
    ...
    \begin{enumerate}
        \item $v \in V(G) \backslash N^{2r}_{G}[J]$. Because $S_1$ is a feasible solution,
        it is clear that $v$ will be dominated by some vertex $u$ in $S_1 \backslash J$. Since 
        $S_1 \backslash J \subseteq S_3$, then $u \in S_3$.
        \item $v \in N^{2r}_{G}[J]$. Because $S_2$ is a feasible solution, it is clear 
        that $S_2 \cap N^{2r}_{G}[J]$ contains a vertex $u$ which dominates $v$. Since 
        $S_2 \cap N^{2r}_{G}[J] \subseteq S_3$, then $u \in S_3$.
    \end{enumerate}
...
\begin{algorithm}[H]
    ...
    a feasible solution of $r$-Dominating Set $S_1$ on $G$
    and a vertex set $J \subseteq V(G)$.
    \ENSURE a feasible solution $S'$ on $G$, such that for all other feasible solutions $S^{*}$, 
    we have $w(S') \leq w(S^{*} \oplus^{2r}_{G,J} S_1)$. 
    \STATE Let $H \leftarrow G[N^{2r}_{G}[J]] \cup \{q\}$, where for vertex $q$
    it holds that $N_H(q) := N_{G}(N^{2r}_{G}[J])$ (Vertex $q$ is adjacent to the vertices 
    at distance exactly $2r$ from $J$). \label{state:construct-H-rds}
    \STATE \[
    w_H(x) = 
    \begin{cases}
    ...
    \IF{ $w(S_2 \oplus^{2r}_{G,J} S_1) < w(S_1)$ }
        \RETURN $S_2 \oplus^{2r}_{G,J} S_1$
    \ELSE
        \RETURN $S_1$
    \ENDIF
    \end{algorithmic}
...
\refstepcounter{definition}
\begin{mydefinition}[$r$-DS-$(2r)$-stitching]{Lemma}
    ...
    $f(\tw) \cdot V(G)^{\mathcal{O}(1)}$ where $\tw$ is the treewidth of $(N^{2r}_{G}[J])$.

\end{mydefinition}

The proof of Lemma~\ref{lemma-rds-stitching} is outside of the scope of this thesis 
...
For some constant $r$ and $\tw$, the treewidth of $N^{2r}_{G}[J]$, the running time of 
