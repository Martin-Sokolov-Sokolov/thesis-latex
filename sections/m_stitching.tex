...
\begin{mydefinition}[FVS-4CL-2-stitching]{Lemma}
    ...
    in time $f(\tw(G)) \cdot |V(G)|^{O(1)}$. Then Algorithm~\ref{alg:stitch-fvs-4cl} solves 
    FVS-4CL-2-stitching in time $f(\tw(N^{2}_{G}[J])) \cdot V(G)^{O(1)}$.
...
\begin{mydefinition}[r-DS-(r+1)-stitching]{Lemma}
    ...
    $(G, w : V(G) \to \mathbb{N})$ in time $f(\tw(G)) \cdot |V(G)|^{O(1)}$. Then 
    Algorithm~\ref{alg:stitch-r-DS} solves $r$-DS in time 
    $f(\tw(N^{r+1}_{G}[J])) \cdot V(G)^{O(1)}$.
