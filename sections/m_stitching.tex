\section{\textit{m-stitching} and \textit{$\Pi$-m-stitching}}

\noindent In this section we initiate the study of certified algorithms for FVS-BCL.
We introduce some key concepts that allow us 
...
in \cite{bumpus_et_al:LIPIcs.SWAT.2024.19, venne2022designing}. 

\noindent Unless specified otherwise, let $\Pi$ be a vertex-minimization problem.

\subsection{Algorithms for \textit{m-stitching} and \textit{$\Pi$-m-stitching}}
...
\begin{mydefinition}[FVS-BCL is 2-stitchable]{Lemma}
    ...
    \noindent Let $(G, w)$ be any instance to the FVS-4CL problem, $J \subseteq V(G)$, some subset
...
\noindent Now we describe an algorithm to find a feasible solution of minimum weight.

\begin{algorithm}[H]
    \caption{Stitch-FVS-4CL}
    \label{alg:stitch-fvs-4cl}
    \begin{algorithmic}[1]
    \REQUIRE Vertex-weighted planar graph $(G, w:V(G) \to \mathbb{N})$, a vertex set $J \subseteq V(G)$ and
    a feasible solution $S_1$ on $G$.
    ...
    \STATE Let $S_2$ be the output of algorithm $A$, described in Lemma~\ref{lemma-stictch-fvs-4cl}, on 
    input $(H, w)$. \label{state:opt-solution}
...
\begin{mydefinition}[FVS-4CL-2-stitching]{Lemma}
    ...
    \noindent Let $A$ be an algorithm that solves FVS-4CL on any vertex-weighted instance $(G, w:V(G) \to \mathbb{N})$
    in time $f(tw(G)) \cdot |V(G)|^{O(1)}$. Then Algorithm~\ref{alg:stitch-fvs-4cl} solves 
    FVS-4CL-2-stitching in time $f(tw(N^{2}_{G}[J])) \cdot V(G)^{O(1)}$.
...
We prove the feasibility of $S_2 \oplus^{2}_{G,J} S_1$ by case analysis on any four vertices $v_1 v_2 v_3 v_4$
    ...
    \begin{figure}[H]
        ...
        \begin{minipage}{0.50\textwidth}
            ...
            \begin{tikzpicture}[scale=0.8]
                ...
                \node[fill=black, circle, inner sep=1.5pt] (j02) at (-0.5,1) {};
                ...
                \draw[blue, thick] (n14) circle(0.5);
                \draw[blue, thick] (n16) circle(0.5);
                \draw[blue, thick] (n23) circle(0.5);
                \draw[blue, thick] (n31) circle(0.5);
...
\noindent The proof of optimality of the set $S_2 \oplus^{2}_{G,J} S_1$ is similar to the one in 
\textbf{Lemma 3.2.} in Bumpus et al.\cite{bumpus_et_al:LIPIcs.SWAT.2024.19}. It is important to note 
...
\noindent Remember we are still trying to prove what we mentioned in Algorithm~\ref{alg:p-m-stitching},
...
\noindent Assume by way of contradiction that there is a feasible solution $S_3$ for $G$,
